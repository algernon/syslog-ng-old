...
void
file_monitor_inotify_destroy(gpointer source)
{
...
void
file_monitor_poll_destroy(gpointer source)
{
...
file_monitor_chk_file(FileMonitor * monitor, MonitorBase *source, const gchar *filename)
{
  ...
  if (g_file_test(path, G_FILE_TEST_EXISTS) &&
      monitor->compiled_pattern && g_pattern_match_string(monitor->compiled_pattern, filename) &&
      ...
      while(source_list)
        {
          if (self->monitor_type == MONITOR_POLL)
            file_monitor_poll_destroy(source_list->data);
          else
            {
#if ENABLE_MONITOR_INOTIFY
              file_monitor_inotify_destroy(source_list->data);
#else
              abort();
#endif
            }
